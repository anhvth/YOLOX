...
for i in range(0, len(cmds), int(args.totalgpu)):
    ...
    if i == 0:
        target_tmux = "run-0"
        tmuxcmd = f"tmux new -s '{target_tmux}' -d 'sh {tmpsh} || echo Done sleep 300 && sleep 300'"
        
    else:
        tmuxcmd = f"tmux new-window -n w{wi} -t {target_tmux}: 'sh {tmpsh} || echo Done Sleep 300 && sleep 300'"
    wi += 1
    print(tmuxcmd)
    # os.system(tmuxcmd)
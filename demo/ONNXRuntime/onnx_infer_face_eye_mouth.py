...
class DrowsinessInfer:
    def __init__(self):
        self.state = [np.zeros([2,1,32], dtype=np.float32) for _ in range(6)]
        self.T = transforms_imagenet_eval(32)
        self.cnnlstm = onnxruntime.InferenceSession('cnnlstm.onnx')

    def get_inps(self, x, lh0, lc0, rh0, rc0, mh0, mc0):
        def _px(x):
    ...
    def prob_viz(self, res, input_frame, cls_names = ['awake', 'drowsy', 'sleepy', 'level'],
